    ...
    <div class="row align-items-start">
      <div class="card mb-3 mt-5">
        <div class="row">
          <div class="col-md-8">
            <div class="produtos">
              <div class="card mb-3 mt-2" v-if="cart.items.length > 0">
                <div class="row g-0" v-for="item in cart.items">
                  ...
                  <div class="col-md-8">
                    <div class="card-body d-flex justify-content-between">
                      ...
                      <p class="card-text">{{ item.quantity }} Un.</p>
                      <p class="card-text">
                        R$
...
export default {
  ...
  methods: {
    ...
    addProduct(productId) {
      OrderDataService.addProduct(this.cart.id, productId, session().token)
        .then((res) => {
          OrderDataService.getOrderByUser(session().user.id, session().token)
            .then((res) => {
    ...
    removeProduct(productId) {
      OrderDataService.removeProduct(this.cart.id, productId, session().token)
        .then((res) => {
          OrderDataService.getOrderByUser(session().user.id, session().token)
            .then((res) => {
              session().cart = res.data;
